/**
 * Gate-calibration runner — READY-rate vs E1a-r2 modal-share anchors.
 *
 * For each of the three r2 prompt configs (A prose-only, B full contract,
 * C partial), build the dispatch prompt exactly as E1a did, run `gruntJudge`
 * N independent times per config, and persist a timestamped artifact that
 * juxtaposes per-config READY rate / malformed count / kicked-back questions
 * with the pinned r2 modal-share anchors. The runner computes NO pass/fail
 * verdict — interpretation is human (see specs/readiness-gate.spec.md,
 * "useful signal").
 *
 * Spec: specs/gate-calibration.spec.md rev 1.
 */
...
import { gruntJudge } from "../gate.ts";
...
export interface GateCalibrationOptions {
...
export interface ConfigCalibration {
  ...
  /** Every verdict's questions, concatenated in call order, duplicates kept. */
  readonly questions: readonly string[];
}

// UNDECIDED: the spec pins runGateCalibration's options but not its return
// type; copied the e1a/e1b idiom ({ deadRun }) since AC5's nonzero-exit CLI
// path needs exactly that bit.
...
export async function runGateCalibration(
  opts: GateCalibrationOptions = {},
): Promise<GateCalibrationResult> {
  const n = opts.n ?? 20;
  ...
  const results = Object.fromEntries(
    CALIBRATION_CONFIGS.map((config) => {
      ...
      for (let i = 0; i < jobs.length; i++) {
        ...
        const v = verdicts[i]!;
        if (v.malformed) {
          // Malformed counts as not-ready (gate dogma fail-closed).
          malformedCount++;
        } else if (v.ready) {
          readyCount++;
        }
      ...
      const result: ConfigCalibration = {
        readyCount,
        readyRate: readyCount / n,
        malformedCount,
        questions,
      };
      return [config, result];
    }),
  ) as Record<CalibrationConfig, ConfigCalibration>;
  ...
  const fileName = `gate-calibration-${ts}.json`;
  await writeFile(join(outDir, fileName), JSON.stringify(artifact, null, 2));

  console.log(`\n=== Gate calibration — ${TASK_NAME} (N=${n} per config) ===\n`);
  for (const config of CALIBRATION_CONFIGS) {
    const r = results[config];
    console.log(
      `Config ${config}: readyRate=${r.readyRate.toFixed(3)} (anchor modal-share ${ANCHORS[config].toFixed(3)})` +
        ` malformed=${r.malformedCount} questions=${r.questions.length}`,
    );
  }
...
if (
  ...
  const opts: GateCalibrationOptions = {
    n: parseInt(getArg("--n", "20"), 10),
    out: getArg("--out", "runs"),
    live: true,
  };

  runGateCalibration(opts)
    .then(({ deadRun }) => {
