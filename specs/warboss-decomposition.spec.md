...
> Status: active · **rev 6** (2026-07-02: the fractal gets a mechanism — `decomposeRecursive(opts & {maxDepth?: 2})`. Rev 5's cap rule said an over-cap intent "must be decomposed further UP the chain" but no code path did it; now the over-cap throw is a typed `RequirementCapExceeded` (count, cap, and `costUsd` stamped by `decompose` so the spent call is not a cost leak), and the wrapper catches exactly that error, issues one `warboss.partition` call (pinned `PARTITION_SYSTEM`; each sub-intent must be self-contained and carry its shared seam verbatim in a `seam` field), parses with the standard one-re-ask policy, requires 2..maxRequirements sub-intents (a 1-element partition is no progress — fail up), recurses sequentially with the seam joined into each child's `context`, and merges children in order — duplicate requirement ids across children throw, `costUsd` totals every call including the failed oversized decompose. Depth exhausted → the cap error propagates (fail-closed). Plain `decompose` is unchanged) · **rev 5** (2026-07-02: amend verification is mechanical — rev 4's implementation counted a gap as addressed when the requirement's example COUNT grew, so one unrelated added example silently cleared every gap on that requirement (a fail-open hole in the one audit→amend round). The amend call now instructs an object output `{"requirements": [...], "pinned": [{"id", "gap", "examples": [<new example names>]}]}`; a gap is addressed iff a pinned entry matches its `id` + `gap` verbatim AND names ≥1 example that exists on the post-amend requirement and was NOT among that requirement's pre-amend example names. Old bare-array amend output remains parseable but earns no credit (fail-closed back-compat: drafts used, all gaps carried); malformed pinned entries are dropped; `validateDrafts` failures on the amended requirements still propagate (the mandate survives the second pass)) · **rev 4** (2026-06-12: underdetermined-semantics kick-back, driven by the E2 measurement `archive/reports/e2-verdict.md` — (1) fiat-flagging: every requirement carries a `resolutions` array naming each semantic choice and whether the intent forced it (`basis: "intent"`) or the warboss chose by fiat (`basis: "fiat"`); fiat entries escalate to Leader; (2) audit gaps are classified `intentDecides: true|false` — intent-undecided gaps are NEVER self-amended, they escalate; (3) `admit` drops the introspective `gruntJudge` from the decision path (three calibration FAILs: `archive/reports/gate-calibration-verdict.md`, `archive/reports/derive-calibration-verdict.md`, E2 admit-in-anger 0-questions miss) — the convergence probe is the only admission instrument, fail-closed when no battery; (4) `maxRequirements` is injected into the decompose prompt, closing the H-14 gap where the cap was post-validation only) · rev 3 2026-06-11: H-9 gaps closed — audit double parse-failure pinned to sentinel (was silent fail-open), `auditGaps` entry format pinned, audit prompt cosmetic fixed to match code; AC10/AC11 added · rev 2 2026-06-10: entropy-reduction mandates in `DECOMPOSE_SYSTEM` · Feature: warboss-decomposition · Added: 2026-06-10 · Maps to: PLAN Phase 4 (warboss decomposition) + E2 substrate
...
18. **AC18 — amend credit is per-gap and mechanical (rev 5).** (a) Amend adds an
    unrelated new example with no `pinned` mapping → the gap appears verbatim in
    `auditGaps` (rev 4's proxy would have cleared it); (b) `pinned` names a
    non-existent example → carried; (c) `pinned` names a PRE-EXISTING example
    name → carried; (d) bare-array amend output (rev-4 shape) with a genuinely
    added example → validated and used as the amended drafts, but ALL amendable
    gaps carried (no credit without a mapping); (e) object shape with a correct
    mapping to a genuinely new example → `auditGaps: []` and the example present
    on the frozen contract.

## Verifies-with

- Tests: `test/warboss.test.ts` — AC1–AC18, offline, scripted fake
...
- Falsifies / experiment link: **E3** (`specs/e3-intent-divergence.spec.md`)
  ...
  (`archive/reports/e2-verdict.md`) is the driver: the kick-back must fire at the
