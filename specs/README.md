...
| [decompose-run](decompose-run.spec.md) | Thin CLI runner around `decompose`+`admit`; timestamped artifact, dead-run guard (rev 2: jsonl sidecar, `--max-requirements` NaN guard, blessed deadRun-omit/CRLF readings) | Phase 4 integration | active |
| [e4-battery-authoring](e4-battery-authoring.spec.md) | E4: Leader answers escalations → warboss re-authors with locked decisions → re-run E2 on a neutral Leader-battery oracle; closes the kick-back loop end-to-end (rev 4: literal-free authoring-diversity hint in the shared renderer so the author stops echoing ruling inputs — candidate #3 ordering happy-lift; rev 3: `extraCases` per ruling so the decimal class survives an author self-echo + owner ordering rulings; rev 2: prose-only owner decisions + single shared cost-ledger) | Phase 4 follow-on (E3 consequence) | active |
| [kickback-pipeline](kickback-pipeline.spec.md) | Production wiring of the E4 loop: live `decompose-run` emits an owner-answer queue from its escalations, owner hand-fills it, a `--reauthor-from`/`--answers` mode folds the decisions back into a fresh decompose; shared `src/kickback.ts` (`renderDecisionBlock` + rev-2 `DECISION_DIVERSITY_HINT`, `loadOwnerAnswers`) | Phase 4 follow-on (E4 consequence #1, rev 2 = #3) | active |
| [multi-task-replication](multi-task-replication.spec.md) | Replicate E4 on a second task (`parse-range`); generalize `loadLeaderAnswers` coverage check to a per-task `contested.json` so `runE4 --task` is task-agnostic | Phase 4 follow-on (E4 consequence #4) | active |
| [probe-signature](probe-signature.spec.md) | Optional task-asset `signature` threaded to the E3 intent probe; closes the H-20 `signature=""` fail-up | Lever 1 / E3 rev 2 | active |
| [intent-probe-viability](intent-probe-viability.spec.md) | `intentProbe` rev 3: split nonviable into `noEntry` vs genuine all-reject; scaffold the default probe system string | Lever 1 / E3 rev 2 | active |
| [e3-needle-matcher](e3-needle-matcher.spec.md) | E3 evaluator rev 2: tighten `E3_NEEDLES` to literals + unambiguous phrases; kill the decimal substring false positive | E3 rev 2 | active |
