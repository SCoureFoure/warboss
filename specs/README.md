...
| [multi-task-replication](multi-task-replication.spec.md) | Replicate E4 on a second task (`parse-range`); generalize `loadGodAnswers` coverage check to a per-task `contested.json` so `runE4 --task` is task-agnostic | Phase 4 follow-on (E4 consequence #4) | active |
