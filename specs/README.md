...
| [e4-battery-authoring](e4-battery-authoring.spec.md) | E4: God answers escalations → warboss re-authors with locked decisions → re-run E2 on a neutral God-battery oracle; closes the kick-back loop end-to-end (rev 3: `extraCases` per ruling so the decimal class survives an author self-echo + owner ordering rulings; rev 2: prose-only owner decisions + single shared cost-ledger) | Phase 4 follow-on (E3 consequence) | active |
| [kickback-pipeline](kickback-pipeline.spec.md) | Production wiring of the E4 loop: live `decompose-run` emits an owner-answer queue from its escalations, owner hand-fills it, a `--reauthor-from`/`--answers` mode folds the decisions back into a fresh decompose; shared `src/kickback.ts` (`renderDecisionBlock`, `loadOwnerAnswers`) | Phase 4 follow-on (E4 consequence #1) | active |
